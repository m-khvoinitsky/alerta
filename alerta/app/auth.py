...
def saml_response_from_idp():
    ...
    for attr, values in app.config.get('ALLOWED_SAML2_ATTRS', {}).items():
        if not any(map(lambda x: x in identity[attr], values)):
            return _make_response({'status': 'error', 'message': 'User does not have required claims'}, 403)
